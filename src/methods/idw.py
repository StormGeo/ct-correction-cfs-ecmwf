...
def great_circle_distance(latitudes, longitudes,
    ...
    lat1, lon1, alt1 = np.radians(latitudes), np.radians(longitudes), altitudes
    lat2, lon2, alt2 = np.radians(grid_lat), np.radians(grid_lon), grid_alt
    dlat = lat2 - lat1
    dlon = lon2 - lon1
...
def idw_weights(latitudes, longitudes, altitudes,
...
def apply_idw_loop(latitudes, longitudes, altitudes, data,
    ...
    for i in range(len(grid_lat)):
        for j in range(len(grid_lon)):
            weights = idw_weights(latitudes, longitudes, altitudes,
                                  grid_lat[i], grid_lon[j], grid_alt[i, j], radius)
            grid[i, j] = np.sum(data * weights) / np.sum(weights)
    return grid


def apply_idw(latitudes, longitudes, altitudes, data,
              grid_lat, grid_lon, grid_alt, args):

    radius = args.interpolation_radius
    logging.info(f"src.methods.idw >> applying IDW interpolation using {radius}km of radius")
    ...
    logging.info(f"src.methods.idw >> IDW interpolation done")
    return grid
